...
class LPIPSDistanceCalculator(BaseMetric): 
    def __init__(self, config=None, device='cuda', resize_size=384):
        super().__init__()
        ...
        self.device = device
        # Standard size for all images (e.g., 224x224)
        self.resize = Resize((resize_size, resize_size), antialias=True)
    ...
    def to_tensor_transform(self, pil_img):
        ...
        return self.normalize(self.to_tensor(self.resize(pil_img)))
