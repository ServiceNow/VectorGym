...
class SVGMetrics: 
    ...
    def calculate_metrics(self, batch, update=True):
        ...
        for metric_name, metric in self.active_metrics.items():
            ...
            if metric_name in ['L2', 'Masked-L2', 'SSIM', 'CLIPScore', 'LPIPS', 'CountTokenLength', 'DinoScore', 'VLMJudge', 'GenerationTime']:
    ...
    def _handle_ratio_metric(self, metric_name, metric, batch, avg_results_dict, all_results_dict):
        ...
        for item in batch['json']:
            ...
            if metric_key == 'non_compiling' and metric_key not in item:
                # Fallback to 'no_compile' if 'non_compiling' is missing
